...
// 自定义函数
ElemType *Txtread(const char *filename);
...
status PriorElem(SqList L, ElemType cur, ElemType &pre_e)
{
	...
	for (; i <= L.length; i++)
	{
		if (L.elem[i - 1] == cur)
		{
			if (i = 1)
			{
				printf("There is no element before cur\n");
			}
			else
			{
				pre_e = L.elem[i - 2];
...
ElemType *Txtread(char const *filename)
{
	// only for integar number in an array
	FILE *pFile;
	long lSize;
	char *buffer;
	size_t result;
