...
VNode * GeneNode(char * ID, char *weight, char *msg);
            ...
            case 5:{
                if(!G){
                    ...
                    if(vex = LoacteVex(G, value, valuetype), vex>=0){
                        ...
                        VNode *pn = GeneNode(ID, weight, msg);
                        pn->firstarc =  G->vertices[vex].firstarc;
                        G->vertices[vex]= *pn;
                        free(pn);
                        printf("顶点已成功赋值\n");
                    }
...
status InitiaG(GPListtype **GL, DiGraph **G){
    ...
    (*GL)->curGP = (*G) = (*GL)->contents[0] = (DiGraph*)malloc(sizeof(DiGraph));
    (*GL)->size = 10;
    (*GL)->num = 1;
    (*G)->name = NULL;
    return OK;
...
int NextAdjVex(DiGraph * G, int v, int w){
    ...
    while(pn&&pn->adjvex!= w){
        pn = pn->nextarc;
    }
    if(pn && pn->nextarc)
        return pn->nextarc->adjvex;
    else return -1;
}
...
status InsertVex(DiGraph *G, VNode * vex){
    ...
    G->vertices[G->vexnum].data = cJSON_Duplicate(vex->data, 1);
...
status InsertArc(DiGraph * G, int tail, int head){
    ...
    if(!pn){
        G->vertices[tail].firstarc = (ArcNode *)malloc(sizeof(ArcNode));
        G->vertices[tail].firstarc->adjvex = head;
        G->vertices[tail].firstarc->nextarc = NULL;
    }
    else{
    while(pn->nextarc){
        if(head == pn->adjvex ){
...
status InfoReadG(GPListtype *GL, char *filename){
    ...
    cJSON * JSONroot = cJSON_Parse(buffer);  free(buffer);
    ...
    for(i = 0; i<size; ++i){
        ...
        for(j = 0; j<GL->curGP->vexnum; ++j){
            //逐个添加节点
             cJSON *vex  = cJSON_GetArrayItem(nodelist, j);
             char * s = cJSON_PrintUnformatted(vex);
             GL->curGP->vertices[j].data = cJSON_Parse(s);
            GL->curGP->vertices[j].firstarc = NULL;
        }
        
        //3.arcs
        cJSON * arcs = cJSON_GetObjectItem(divg, "arcs");
        GL->curGP->arcnum = cJSON_GetArraySize(arcs);
        for(j = 0; j< GL->curGP->arcnum; ++j){
            cJSON * divarcs = cJSON_GetArrayItem(arcs, j);
            int archead = LoacteVex(GL->curGP, cJSON_GetObjectItem(divarcs, "head")->valuestring, 1);
            int arctail = LoacteVex(GL->curGP, cJSON_GetObjectItem(divarcs, "tail")->valuestring, 1);//这里可能有类型转换错误
...
status InfoWriteG(GPListtype * GL, char *filename){
    ...
    for(i = 0; i<num ;++i){
        ...
        //1.add name
        const char * divname = GL->contents[i]->name;
        cJSON_AddStringToObject(divG, "name", divname);
         #ifdef DEBUG
            char * out1 = cJSON_PrintUnformatted(divG);
            printf("the first G after adding name\n%s\n", out1);
        #endif
        //2. add vertices
        cJSON * divvex;
        divvex = cJSON_CreateArray();
        int vexnum = GL->contents[i]->vexnum;
        int j;
        for(j = 0; j< vexnum ; ++j){
            cJSON_AddItemToArray(divvex, GL->contents[i]->vertices[j].data);
            #ifdef DEBUG
            char * out1 = cJSON_PrintUnformatted(GL->contents[i]->vertices[j].data);
            printf("data of the vex j\n%s\n", out1);
            #endif
        }
        ...
        for(j =0; j<vexnum; ++j){
            ArcNode * arcto = GL->contents[i]->vertices[j].firstarc;
            const char * tail = (cJSON_GetObjectItem(GL->contents[i]->vertices[j].data, "ID")->valuestring);
            #ifdef DEBUG
            char * out1 = cJSON_PrintUnformatted(GL->contents[i]->vertices[j].data);
            printf("%s\n", out1);
...
#ifdef DEBUG
                char *out2 = cJSON_PrintUnformatted(GL->contents[i]->vertices[arcto->adjvex].data);
                printf("%s\n", out2);
#endif
//这个输出会出现错位的重读式信息
                const char *head = (cJSON_GetObjectItem(GL->contents[i]->vertices[arcto->adjvex].data, "ID")->valuestring);
                cJSON_AddStringToObject(divarc, "head", head);
                cJSON_AddStringToObject(divarc, "tail", tail);
...
VNode * GeneNode(char * ID, char *weight, char *msg){
    VNode * e = (VNode*)malloc(sizeof(VNode));
